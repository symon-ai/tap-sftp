...
class SFTPConnection():
    ...
    def get_files(self, prefix, search_pattern, modified_since=None, search_subdirectories=True):
        ...
        if files:
            LOGGER.info('Found %s files in "%s"', len(files), helper.sanitize_for_log(prefix))
        else:
            LOGGER.warning(
                'Found no files on specified SFTP server at "%s"', helper.sanitize_for_log(prefix))
        ...
        if matching_files:
            LOGGER.info('Found %s files in "%s" matching "%s"',
                        len(matching_files), helper.sanitize_for_log(prefix), helper.sanitize_for_log(search_pattern))
        else:
    ...
    def get_file_handle(self, f, file_type, encoding, decryption_configs=None):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            ...
            if decryption_configs:
                ...
                LOGGER.info('Decrypting file: %s', helper.sanitize_for_log(sftp_file_path))
    ...
    def _get_text_encoding(local_path, configured_encoding, file_size=None):
        ...
        LOGGER.info(
            ...
            helper.sanitize_for_log(local_path),
            local_file_size
        )
    ...
    def _download_file_with_bounded_prefetch(self, sftp_file_path, local_path, file_size=None):
        ...
        LOGGER.info(
            ...
            helper.sanitize_for_log(sftp_file_path),
            helper.sanitize_for_log(local_path),
            file_size,
    ...
    def get_files_matching_pattern(self, files, pattern):
        ...
        LOGGER.info(f"Searching for files for matching pattern: {helper.sanitize_for_log(pattern)}")
