...
class Transformer:
    ...
    def _get_clean_num_str(self, data):
        # An accounting column in excel, stores 0 as $- or $(-)
        data = '0' if data == '$-' else data
        data = '0' if data == '($-)' else data
        data = '' if data == '-' else data

        # replace commas + signs
        data = data.replace(',', '')
        data = data.replace('$', '')
        data = data.replace('%', '')

	    # Replace open parenthesis ( with negative sign. In accounting, (##) is used to denote a negative number.
        data = data.replace(')', '')
        data = data.replace('(', '-')
    ...
    def _get_transformvalue_by_type(self, data, type):
        if type == 'string':
            ...
            if isinstance(data, str):
                data = self._get_clean_num_str(data)
            ...
            if isinstance(data, str):
                trueList = ['yes', 'true', '1']
                falseList = ['no', 'false', '0']

                lower_val = data.lower()
                if lower_val in trueList:
                    return True, True
                elif lower_val in falseList:
                    return True, False
