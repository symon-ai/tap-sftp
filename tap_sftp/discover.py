    ...
    for table_spec in tables:
        ...
        for f in sorted_files:
            ...
            if file_type in ["csv", "text"]:
                ...
                file = conn.get_file_handle_for_sample(f, file_type, decryption_configs, sample_size)
                with file[0] as file_handle:
                    ...
                    streams += csv_client.build_streams(
                        file_handle, file[1], defaults.SAMPLE_SIZE, tap_stream_id=table_name)
            elif file_type in ["excel"]:
                with conn.get_file_handle(f, file_type, decryption_configs) as file_handle:
                ...
                table_name = table_spec.get('table_name')
                with conn.get_file_handle_for_sample(f,file_type, None, defaults.SAMPLE_SIZE) as file_handle:
